...
std::vector<Aktion::GUI::NodeAddCategory> GetNodeLogicAddList()
{
  std::vector<Aktion::GUI::NodeAddCategory> r = std::vector<Aktion::GUI::NodeAddCategory>();
  std::vector<Aktion::GUI::NodeAddType> event_list = std::vector<Aktion::GUI::NodeAddType>();
  event_list.push_back({ "Start", "", Aktion::Database::Spawn_Start });
  event_list.push_back({ "Update", "", Aktion::Database::Spawn_Update });
  event_list.push_back({ "Key", "", Aktion::Database::Spawn_Key });
  event_list.push_back({ "Keydown", "", Aktion::Database::Spawn_Keydown });
  event_list.push_back({ "Keyup", "", Aktion::Database::Spawn_Keyup });
  std::vector<Aktion::GUI::NodeAddType> logic_list = std::vector<Aktion::GUI::NodeAddType>();
  logic_list.push_back({ "If", "", Aktion::Database::Spawn_If });
  logic_list.push_back({ "And", "", Aktion::Database::Spawn_And });
  logic_list.push_back({ "Or", "", Aktion::Database::Spawn_Or });
  logic_list.push_back({ "Not", "", Aktion::Database::Spawn_Not });
  std::vector<Aktion::GUI::NodeAddType> utility_list = std::vector<Aktion::GUI::NodeAddType>();
  utility_list.push_back({ "Print", "", Aktion::Database::Spawn_PrintMessage });
  std::vector<Aktion::GUI::NodeAddType> math_list = std::vector<Aktion::GUI::NodeAddType>();
  math_list.push_back({ "Add", "", Aktion::Database::Spawn_Addition });
  math_list.push_back({ "Subtract", "", Aktion::Database::Spawn_Subtraction });
  math_list.push_back({ "Multiply", "", Aktion::Database::Spawn_Multiply });
  math_list.push_back({ "Division", "", Aktion::Database::Spawn_Division });
  math_list.push_back({ "Power", "", Aktion::Database::Spawn_Power });
  std::vector<Aktion::GUI::NodeAddType> compare_list = std::vector<Aktion::GUI::NodeAddType>();
  compare_list.push_back({ ">", "", Aktion::Database::Spawn_Greater });
  compare_list.push_back({ ">=", "", Aktion::Database::Spawn_Greater_Equal });
  compare_list.push_back({ "<", "", Aktion::Database::Spawn_Less });
  compare_list.push_back({ "<=", "", Aktion::Database::Spawn_Less_Equal });
  compare_list.push_back({ "==", "", Aktion::Database::Spawn_Equal });  
  r.push_back({ "Event", "", event_list });
  r.push_back({ "Logic", "", logic_list });
  r.push_back({ "Utility", "", utility_list });
  r.push_back({ "Math", "", math_list });
  r.push_back({ "Compare", "", compare_list });
  return r;
}

void Aktion::GUI::GUI_Logic_PopupAddNode(Aktion::GUI::AktionGUI& host, NodeEditorBuffer& target)
{
  ...
  if (ImGui::BeginPopupModal("Create Logic Node", &target.m_ShowAddNode, ImGuiWindowFlags_AlwaysAutoResize | ImGuiWindowFlags_NoMove | ImGuiWindowFlags_NoResize))
  {
    auto newNodePostion = openPopupPosition;
    Node* node = nullptr;
    if (target.m_NodeAddBuffer.size() == 0 || target.m_UpdateNodeList)
      target.m_NodeAddBuffer = GetNodeLogicAddList();
    ...
    if (node) {
      ...
      host.m_Logic.m_ShowAddNode = false;
...
void Aktion::GUI::GUI_Logic_InitLogicEditor(NodeEditorBuffer& target)
{
  ...
  Node* n = Aktion::Database::Spawn_Start();
  NodeUtility_BuildNode(n);
  ax::NodeEditor::SetNodePosition(target.m_Nodes.back().ID, ImVec2(0, 0));
  ax::NodeEditor::NavigateToContent();
}
void Aktion::GUI::GUI_Logic_DrawLogicEditor(Aktion::GUI::AktionGUI& target)
{
  NodeUtility_SetBuffer(target.m_Logic);
  Aktion::Database::Node_Database(&target.m_Logic.m_Nodes, NodeUtility_BuildNode, NodeUtility_GetNextId);
  ax::NodeEditor::Begin("My Editor");
  {
    auto cursorTopLeft = ImGui::GetCursorScreenPos();

    ax::NodeEditor::Utilities::BlueprintNodeBuilder builder(target.m_Logic.m_HeaderBackground, GUI_GetTextureWidth(target.m_Logic.m_HeaderBackground), GUI_GetTextureHeight(target.m_Logic.m_HeaderBackground));

    for (auto& node : target.m_Logic.m_Nodes)
    {
      ...
      if (!isSimple)
      {
        ...
        if (hasOutputDelegates)
        {
          ...
          for (auto& output : node.Outputs)
          {
            ...
            if (target.m_Logic.m_NewLinkPin && !NodeUtility_CanCreateLink(target.m_Logic.m_NewLinkPin, &output) && &output != target.m_Logic.m_NewLinkPin)
      ...
      {
        auto alpha = ImGui::GetStyle().Alpha;
        if (target.m_Logic.m_NewLinkPin && !NodeUtility_CanCreateLink(target.m_Logic.m_NewLinkPin, &input) && &input != target.m_Logic.m_NewLinkPin)
          alpha = alpha * (48.0f / 255.0f);
    ...
    for (auto& node : target.m_Logic.m_Nodes)
    {
      ...
      if (!node.Inputs.empty())
      {
        ...
        if (target.m_Logic.m_NewLinkPin && !NodeUtility_CanCreateLink(target.m_Logic.m_NewLinkPin, &pin) && &pin != target.m_Logic.m_NewLinkPin)
    ...
    if (!target.m_Logic.m_CreateNewNode)
    {
      if (ax::NodeEditor::BeginCreate(ImColor(255, 255, 255), 2.0f))
      {
        ...
        if (ax::NodeEditor::QueryNewLink(&startPinId, &endPinId))
        {
          ...
          target.m_Logic.m_NewLinkPin = startPin ? startPin : endPin;
          ...
          if (startPin && endPin)
          {
            ...
            else
            {
              ...
              {
                target.m_Logic.m_Links.emplace_back(Link(NodeUtility_GetNextId(), startPinId, endPinId));
                target.m_Logic.m_Links.back().Color = Aktion::Database::GetIconColor(startPin->Type);
              }
            }
          }
        }
        ...
        {
          target.m_Logic.m_NewLinkPin = NodeUtility_FindPin(pinId);
          if (target.m_Logic.m_NewLinkPin)
            showLabel("+ Create Node", ImColor(32, 45, 32, 180));

          if (ax::NodeEditor::AcceptNewItem())
          {
            target.m_Logic.m_CreateNewNode = true;
            target.m_Logic.m_NewNodeLinkPin = NodeUtility_FindPin(pinId);
            target.m_Logic.m_NewLinkPin = nullptr;
      ...
      if (ax::NodeEditor::BeginDelete())
      {
        ...
        while (ax::NodeEditor::QueryDeletedNode(&nodeId))
        {
          ...
          {
            auto id = std::find_if(target.m_Logic.m_Nodes.begin(), target.m_Logic.m_Nodes.end(), [nodeId](auto& node) { return node.ID == nodeId; });
            if (id != target.m_Logic.m_Nodes.end())
              target.m_Logic.m_Nodes.erase(id);
          }
        }
        ...
        while (ax::NodeEditor::QueryDeletedLink(&linkId))
        {
          if (ax::NodeEditor::AcceptDeletedItem())
          {
            auto id = std::find_if(target.m_Logic.m_Links.begin(), target.m_Logic.m_Links.end(), [linkId](auto& link) { return link.ID == linkId; });
            if (id != target.m_Logic.m_Links.end())
              target.m_Logic.m_Links.erase(id);
          }
        }
      }
